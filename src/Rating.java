    ...
    public static int rateAttack() {
        ...
        for (int i=0;i<64;i++) {
            switch (HadesChess.chessBoard[i/8][i%8]) {
                case "P": {HadesChess.kingPositionC=i; if (!HadesChess.kingSafe()) {counter-=64;}}
                break;
                case "R": {HadesChess.kingPositionC=i; if (!HadesChess.kingSafe()) {counter-=500;}}
                break;
                case "K": {HadesChess.kingPositionC=i; if (!HadesChess.kingSafe()) {counter-=300;}}
                break;
                case "B": {HadesChess.kingPositionC=i; if (!HadesChess.kingSafe()) {counter-=300;}}
                break;
                case "Q": {HadesChess.kingPositionC=i; if (!HadesChess.kingSafe()) {counter-=900;}}
                break;
            }
        }
        HadesChess.kingPositionC=tempPositionC;
        if (!HadesChess.kingSafe()) {counter-=200;}
        return counter/2;
    }
        ...
        for (int i=0;i<64;i++) {
            switch (HadesChess.chessBoard[i/8][i%8]) {
                ...
                case "Q": counter+=queenBoard[i/8][i%8];
                    break;
                case "A": if (material>=1750) {counter+=kingMidBoard[i/8][i%8]; counter+=HadesChess.posibleA(HadesChess.kingPositionC).length()*10;} else
                {counter+=kingEndBoard[i/8][i%8]; counter+=HadesChess.posibleA(HadesChess.kingPositionC).length()*30;}
                    break;
            }
        }
        return counter;
